...
| `frontend/src/*.jsx` | UI: `App.jsx` shell, `Dashboard.jsx`, `Fleet.jsx`, `Workflows.jsx`/`WorkflowCanvas.jsx`, `AddAgent.jsx`, `Settings.jsx`, `AskVisuals.jsx`, `CostPage.jsx`. `JobDetail.jsx` is the Run (page + Home desk panel); its page has two views under one header — **Run** (steps, passes, Visibility, Evidence) and **Execution** (`ExecutionView.jsx` + pure `execution.js`): a vertical tree drawn from `GET /work/items/{id}/execution` parent ids only, a right-side type-aware inspector, and a chronology fallback used solely when the endpoint recorded no `attached` parent among two or more nodes. The client never infers parentage, types, retries, success or cost; several roots and traces are normal. |
...
- **Two representations of the same work, not two sources of truth.** A run has one
  ...
  **Execution Graph** (`work_execution.py`) is the technical resolution — which worker ran,
  ...
  the loop event, Evidence record and Execution node it came from. The Work Graph is
