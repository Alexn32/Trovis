...
- **Work home (lean):** `GET /work/overview` (counts) + `GET /work/items` (paginated named rows) + `GET /workflows` (declared jobs). Three views of the same rows — By job (`WorkTab.jsx` `JobRow`: a job row shows only the runs that need a person, the rest is a state bar, a count and a fold; a Rows/List density toggle, remembered per browser, swaps the rows for one line per job via `workPage.compactJobLine`), All open (the one `WorkTable`) and Completed (`GET /work/items?status=done` + the 7-day `GET /home/snapshot` series, fetched only while that view is open); presentation rules in `frontend/src/workPage.js`. **There is one job page**, `/work/jobs/:id` (`WorkTab.jsx` `JobPage`), laid out in the agent detail page's language (`.jp` in `styles.css`: the same full-bleed linen shell, cream cards, mono labels, dot-pills, feed rows — and its page-scoped light palette, by the same decision `AgentDetail.jsx` carries), so a job and an agent read as two pages of one product: definition, four facts, the observed path, the DECLARED steps (kept apart so a declaration never reads as a measurement), health, the job's runs paged by cursor with the board's status chips, settings, version history, and the one edit door (`Edit job`, or `Describe this job` on a derived one). Home's by-job bars and the editor's save both route there; `WorkflowPage.jsx` (the older overlay) is no longer linked. Suggestions: `GET /work/suggestions` + approve/edit/decline. Do **not** call `GET /work/board` or `GET /work/summary` from home — those scan every open loop and starve a single Uvicorn replica. `/health` is a DB-free fastpath.
...
- **Connect (one door):** the header's **+ Connect** (`aria-label="Connect to Trovis"`) opens the `add` overlay: `AddAgent.jsx` `ConnectLanding` ("What do you want Trovis to see?" — a sentence, or an intent chip from `connectSetup.intentChips()`) → `ConnectGuide.jsx`, the guided AI setup, which is the heart of the flow. The guide's opening chips are the registry's AI `guide_label`s **plus the work systems**; a work-system pick (or a `connectors` id in a `POST /connect/ask` reply) renders `WorkSystemConnectCard` inline — the same OAuth door as the Connections page, from `saasDoors.js` — with no model turn, because a work system is a button, never code. The "which one?" intents answer locally from the registry (`initialLocalTurn`). The manual wizard and every recipe page are unchanged underneath ("set up an agent manually"). Two one-shot query flags are read at boot and cleared: `/connections?connect=<id>` (`route.connectRequest`) opens the guide on that connector — the hook a "Connect Stripe" link elsewhere uses — and `/?saas=<provider>_connected|_error` (`route.saasReturn`) lands on Connections. The overlay is keyed by a `nonce` so opening Connect on a connector while the landing is up remounts the flow. Pinned by `connectGuide.test.mjs`, `saasDoors.test.mjs`, `connectSetup.test.mjs`.
- **Connect verification (Phase 5):** the guided setup and the manual wizard no longer decide "connected" by diffing `/agents` service names. The moment a telemetry connector is chosen (guide) or a recipe is on screen (wizard), the client records an instance (`POST /connect/connections`, or resumes the newest `started` row), stamps its `connection_key` into every snippet as `trovis.connection.id` (`connectSnippets.withConnectionKey` fills `TROVIS_CONNECTION_ID`, or DROPS the carrying line when there is no key — a snippet never ships the placeholder; the Grok Bot MCP URL gets `?connection=`), and polls `GET /connect/connections/{id}/status` (`connect_health.build_connection_status` over `database.get_observations_since`, bounded, `since` defaulting to the recorded setup start). The read attributes by the instance key first, then by the connector's stamps, and says which (`attribution: instance | connector`) plus what the spans carried (`sees`: execution / actions / model_usage / named_work — span facts, not Coverage; handoffs and external outcomes are deliberately absent). `connectFlow.lifecycle` words it (set up → waiting for data → connected, and the honest middle "telemetry is arriving without this setup's id"), `connectFlow.cannotSee` names external outcomes as a capability gap of AI connectors with the work systems that add it — capability language, never a verdict. The wizard's `ConnectionStatusStrip` offers "I've run it" (`POST …/complete`); `connectionsPage.aiRowState` reads `instances` to show "Setup started · waiting for data". Embedded onboarding records no instance.
...
| `connection_instances` (in `database.py`) | The **durable record that a connection was set up** — one row per configured instance of a connector (`connectors.py` names the kinds). Store recorded ACTIONS, derive state: `setup_status` is what a person or a door did (`started` / `completed` / `disconnected`); whether data ever arrived is read at request time from spans (telemetry, matched by the `trovis.connection.id` resource stamp = `connection_key`) or `saas_events` (OAuth), never stored. `connect_health` derives the per-instance state (`INSTANCE_STATES`: `setup_started` / `waiting_for_data` / `connected` / `disconnected`, a separate enum — the connector-level `STATES` stay three-valued) and lists them as `instances` on each health row; a `completed` telemetry instance with nothing observed makes its connector read `waiting_for_data`, a first for telemetry. Routes: `POST /connect/connections` (telemetry connectors only — an OAuth door records its own instance when the provider authorizes, via `sync_saas_connection_instance` from `upsert_saas_connection` / `disconnect_saas_connection`), `GET /connect/connections`, `POST /connect/connections/{id}/complete` and `/disconnect` (cross-account ids 404). The key is **not a credential**: lookups are always `(account_id, connection_key)` and a foreign key attributes nothing. Carriers today: the SDK (`init(connection_id=…)` / `TROVIS_CONNECTION_ID` → resource attr), the Grok Bot MCP URL (`/mcp/grok?connection=cn_…`); unstamped telemetry attributes at connector level exactly as before — never a fallback that invents an instance. Not `agent_connections` (`/connections`, Agent Flow edges), a different thing with an older name. |
...
- **Every NAMED run belongs to a job.** Ingest matches each new run against the declared jobs' hint sets; when none claims it **and the run is named work** (`loops.title_source = 'provided'` — the same line `/work/overview` and `/work/items` draw; the untitled OTel flood is not work anyone named and earns no job, so an agent that only emits untitled traces stays an agent on the Agents page), `database._ensure_derived_workflow` files it under a job DERIVED from its agent's `service.name` (one per account + service, enforced by the partial unique index `idx_workflows_derived`; `workflows.derived_from` names the service, NULL means declared). A derived job is named after the agent, carries one hint (`service_name equals`) and NO expectation, so it is observed and never graded; `WorkflowSummary.derived` is what lets the UI say "not yet described". `loops.match_workflow` prefers a declaration over a derived job at equal specificity, so declaring a job takes its runs on the next pass (open runs re-match; closed runs stay frozen where they closed). Promotion is `POST /workflows/{id}/versions` with `name`: the person's description clears `derived_from` and names the job — the only path that ever changes a job's name, and 400 on a declared one. Archiving a derived job is respected: that agent's new runs stay unfiled rather than re-creating it. `_archive_unnamed_derived_jobs` runs at boot and retires derived jobs holding no named run (the ones created before this gate). Telemetry is never refused for lacking a job.
