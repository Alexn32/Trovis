...
// not observability, not Fleet/Cost, not “Connect is live”, not Monday-of.
...
for (const [name, src] of [
  ...
  test(`${name} ships the locked founding copy`, () => {
    ...
    assert.match(src, /where handoffs die/)
    ...
    assert.match(src, /Work as jobs you can see and judge/)
...
test('index.html SEO describes the waitlist', () => {
  ...
  assert.match(index, /where handoffs die/)
