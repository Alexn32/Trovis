...
test('1/2. Execution is a view under the Run: the switch sits under the header, the title, status and back link stay, and the fetch is per view', async () => {
  ...
  await m.settle()
  assert.ok(tab(m, 'Activity') && tab(m, 'Execution'), 'Run | Execution switch present')
  assert.equal(tab(m, 'Activity').getAttribute('aria-selected'), 'true')
  ...
  assert.ok(m.$('.jobd-views'), 'the view switch stays in the header')
