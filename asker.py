...
_SETUP_KNOWLEDGE = (
    ...
    "on the GPT via OAuth, and the GPT reports its own activity. It's "
    "report-only — the GPT shows up in the fleet but can't query it back, and "
    "there's no token-level cost for GPT-Action agents (they run on OpenAI's "
    "side).\n"
...
_CONNECT_SETUP_EXTRAS = (
    ...
    "connectAgent at the start, logActivity per step, and reportComplete when "
    "done; (5) save and run — it authorizes once, then appears in the fleet. "
