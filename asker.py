...
_SETUP_KNOWLEDGE = (
    ...
    "Anthropic Managed Agents), Grok (xAI SDK), a Cursor Grok Bot, and a "
    ...
    "\"Cursor Grok Bot\" is a desktop assistant that exports nothing, so it "
...
_CONNECT_SETUP_EXTRAS = (
    ...
    "- Cursor Grok Bot (a desktop assistant, NOT the xai-sdk path): no pip, "
    "no code. Add Trovis as an MCP server in the Bot: URL "
    "TROVIS_MCP_URL with header 'Authorization: Bearer "
    "TROVIS_API_KEY' (clients that cannot set headers pass api_key on each "
    "tool call). Then paste reporting instructions into the Bot so it calls "
    "report_job_started (with a plain-English title — that title IS the job "
    "name on Work), report_job_waiting when it stops to ask the user "
...
SYSTEM_CONNECT = (
    ...
    "\"Cursor Grok Bot\" for an assistant they chat with in Cursor. Never "
